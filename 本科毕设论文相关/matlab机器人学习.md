...
tags:
  - 本科毕设论文相关
date: 2025-11-30 21:06:00
---
﻿


# 机器人导论学习![img](3084836-20251205203959600-2058504371.png))
## 第一章
### 正运动学和逆运动学![img](3084836-20251205204050886-1461675935.png))![img](3084836-20251205204113798-1196318728.png))![img](3084836-20251205204128357-1998089245.png))
### 关节空间和笛卡尔空间
...
#### 轨迹生成![img](3084836-20251205204617998-491314622.png))
...
特殊处理：基座坐标系 0 位于第一个关节轴线上，末端执行器坐标系 n 固定在最后一个关节轴线上。  ![img](3084836-20251205205433550-1807192610.png))
- - -
此外还有___六参数Stone模型、CPC模型、modified CPC模型___

### 机器人工作空间分析
机器人的工作空间是指机器人在完成所有可到达的位置时，末端执行器包括的 总体空间体积。   
常用的工作空间分析法：数值法、几何法、解析解法  ![img](3084836-20251205210448820-1463338456.png))![img](3084836-20251205210540841-439956487.png))
...
### 动力学参数辨识原理
动力学参数辨识原理  
参数辨识流程图：  ![img](3084836-20251205211120193-826032149.png))
...
___因为机器人结构的物理约束，动力学参数并非任意取值，而是存在参数的上下限___![img](3084836-20251205230444882-795331794.png))
为了便于参数辨识，起始状态和终止状态角度相同，启停位置处角速度与角加速度数值均应设置为0  ![img](3084836-20251205230530262-117058669.png))
___观测矩阵 Y___ 的条件数是评判激励轨迹在辨识过程中的抗噪能力的重 要指标。（轨迹观测矩阵的条件数越小，激励轨迹的抗噪能力越强）
条件![img](3084836-20251205230634189-469645968.png))
...
（通过求导的方法找到损失函数 L(w, b) 的最小值。将 L(w, b) 分别对 w 和 b 求偏导，并令其等于零）![img](3084836-20251205230759847-644115984.png))
...
引入权重矩阵对LS法进行改善（权重矩阵可以由机器人关节的测量力矩估计误差的协方差进行表征）  ![img](3084836-20251205230824838-1854511556.png))
...
__改进遗传算法：__![img](3084836-20251205231036228-238830625.png))
...
![img](3084836-20251130210918207-494710285.jpg))

补充解释：  
旋转矩阵描述了旋转，没有平移![img](3084836-20251130213741300-666269156.png))
绕当前坐标系（“本体坐标系”“动系”）旋转，是每次旋转后坐标系跟着转动；  
绕固定坐标系（也叫 “惯性坐标系”“静系”）旋转，是每次旋转都基于最初的坐标系。![img](3084836-20251130211627179-719296801.png))
左乘是 “相对于固定坐标系” 做变换，右乘是 “相对于当前（本体）坐标系” 做变换  ![img](3084836-20251130211957626-1956672423.png))
- - -
```matlab
...
- - -
补充解释：  
变换矩阵描述了旋转+平移（包含了旋转矩阵的信息）![img](3084836-20251130215809985-453505759.png))  
...
>.dh：D-H 参数表（标准或改进型）。  
-  - -
2、 关键方法  ![img](3084836-20251130220852144-1269411690.png))
...
- - -![img](3084836-20251130223544881-1233950706.png))
...
```![img](3084836-20251130225528530-529801889.png))![img](3084836-20251130230213947-1212088938.png))